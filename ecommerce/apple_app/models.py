...
class AvailabelSize(models.Model):
...
class Product(models.Model):
    ...
    label = models.CharField(max_length=200, choices=_LABEL, blank=True, null=True)
    image = models.ImageField(upload_to='product/img')
    in_stock = models.IntegerField(default=0, blank=True, null=True)
    available_color = models.CharField(max_length=200, null=True, blank=True)
    available_size = models.ManyToManyField(AvailabelSize, related_name='available_sizes')
    ...
    def get_add_to_cart_url(self):
        return reverse("apple:add-to-cart", kwargs={
            'slug': self.slug
        })

    def get_remove_from_cart_url(self):
        return reverse("apple:remove-from-cart", kwargs={
            'slug': self.slug
        })
